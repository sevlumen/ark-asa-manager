...
$scripts = @(
    ...
    'tests/acceptance/instance_delete.ps1',
...
$probeVolumes = @(docker volume ls --format '{{.Name}}' | Where-Object { $_ -match 'rbac-probe|isolation-[ab]-|backup-probe|delete-probe|node-delete-probe|remote-enrollment-probe' })
$probeContainers = @(docker ps -a --format '{{.Names}}' | Where-Object { $_ -match '^ark-(rbac-probe|isolation-[ab]-|backup-probe|delete-probe|node-delete-probe|remote-enrollment-probe)' })
